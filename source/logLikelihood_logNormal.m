function [logLikelihood] = logLikelihood_logNormal(theta, loglike_args)
...
% Mean of distribution needs to be corrected from model
mu = log((y.^2) ./ sqrt(y.^2 + sigma.^2));
% Correct sigma for the lognormal
sigma = sqrt(log(1 + (sigma.^2) ./ (y.^2)));

%log(normpdf(ydat,y,sigma))
%logLikelihood = -sum(log(normpdf(ydat,y,sigma)), 'all');

logLikelihood = sum(-(log(ydat)-mu).^2 ./ sigma.^2/2 - log(ydat) - log(sigma) - log(sqrt(2*pi)),'all');
