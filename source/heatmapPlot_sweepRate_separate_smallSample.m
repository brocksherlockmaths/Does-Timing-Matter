...
[r,c] = find(paramSpace(1,:) == 0.1 & paramSpace(2,:) == 1);
...
filePrefix = 'results_heatmap_smallSample/results_timeError_lognormalData_heatMapCalc_paramSweep';
...
sample = 2; % which set of sampled data we use
% % Loop through each param set
for i = 1:length(indices)
    load(strcat(filePrefix,num2str(indices(i)),'.mat'),'lowerBound','upperBound','MLE','truthContained','trueVals','SIGMA_SET');
    thisTrueVals = [repmat(trueVals,numGridpoints^2,1),SIGMA_SET];
    allTrueVals(:,:,i) = thisTrueVals(:,:);
    intervalWidth(:,:,i) = upperBound(:,:,sample) - lowerBound(:,:,sample);
    mlEstimate(:,:,i) = MLE(:,:,sample);
    intervalMidpoint = (lowerBound(:,:,sample) + upperBound(:,:,sample)) / 2;
    intervalContained_cont1(:,:,i) = abs(thisTrueVals(:,:) - intervalMidpoint) ./ intervalWidth(:,:,i);
    intervalContained_cont2(:,:,i) = min(abs(thisTrueVals(:,:) - lowerBound(:,:,sample)), abs(thisTrueVals(:,:)-upperBound(:,:,sample))); % distance to closest interval bound
    intervalContained_cont2(truthContained == 1) = 0;
end

% Average over all the different samples
% for i = 1:length(indices)
%     load(strcat(filePrefix,num2str(indices(i)),'.mat'),'lowerBound','upperBound','MLE','truthContained','trueVals','SIGMA_SET');
%     thisTrueVals = [repmat(trueVals,numGridpoints^2,1),SIGMA_SET];
%     allTrueVals(:,:,i) = thisTrueVals(:,:);
%     truthContained = isbetween(thisTrueVals,mean(lowerBound,3),mean(upperBound,3));
%     intervalWidth(:,:,i) = mean(upperBound(:,:,:) - lowerBound(:,:,:),3);
%     mlEstimate(:,:,i) = mean(MLE,3);
%     intervalMidpoint = mean((lowerBound + upperBound) / 2,3);
%     intervalContained_cont1(:,:,i) = abs(thisTrueVals(:,:) - intervalMidpoint) ./ intervalWidth(:,:,i);
%     tmpIntervalContained = mean(min(abs(thisTrueVals(:,:) - lowerBound), abs(thisTrueVals(:,:)-upperBound)),3); % distance to closest interval bound
...
for j = 1:length(indices)
    for i = 1:num_params-1
        ...
        if i == 1
            title(strcat("$k = ", num2str(k_range(j)),'$'));
        end
    end
end
...
% Get axes handles in tile order
ax = findall(t1, 'Type', 'axes');
ax = flipud(ax); % findall returns reverse creation order

% Column-major indexing:
leftColIdx   = 1:nRows;                          % first column's tiles
bottomRowIdx = nRows:nRows:(nRows*nCols);        % bottom row's tiles
...
% Make all tick marks face outward
set(findall(t1, 'Type', 'axes'), 'TickDir', 'out');
...
set(findobj(gcf,'type','axes'), 'FontSize', 16, 'LineWidth', 1);
%exportgraphics(t1, 'Figures_withDynamics/heatmap_sweepRate_intervalWidth_top.png', 'Resolution', 300);
set(fig1, 'PaperPositionMode', 'auto');
print(fig1, 'Figures_withDynamics/heatmap_sweepRate_smallSample_intervalWidth_top.png', ...
      '-dpng', '-r300');

% === BOTTOM FIGURE ===
fig2 = figure(2);
fig2.Units = 'centimeters';
fig2.Position = [2, 2, fig1.Position(3), tileSize_cm * 1]; % 1 row height

t2 = tiledlayout(fig2, 1, nCols, 'TileIndexing', 'columnMajor');
...
% After plotting Fig 2's tiles:
% After plotting Fig 2's tiles:
cb2 = colorbar;
cb2.Layout.Tile = 'east';  % same as Fig 1
cb2.Visible = 'off';       % keep it invisible

% After both layouts are created
t2.InnerPosition(1) = t1.InnerPosition(1); % same left edge
t2.InnerPosition(3) = t1.InnerPosition(3); % same width
outerPos = t2.OuterPosition;
outerPos(2) = outerPos(2) - 0.05; % shift down
outerPos(4) = outerPos(4) + 0.05; % increase height
t2.OuterPosition = outerPos;
% Make all tick marks face outward
set(findall(t2, 'Type', 'axes'), 'TickDir', 'out');
...
%print(fig, 'Figures_withDynamics/heatmap_sweepRate_intervalWidth_bottom.png', '-dpng', '-r600');
%exportgraphics(t2, 'Figures_withDynamics/heatmap_sweepRate_intervalWidth_bottom.png', 'Resolution', 300);
set(fig2, 'PaperPositionMode', 'auto');
print(fig2, 'Figures_withDynamics/heatmap_sweepRate_smallSample_intervalWidth_bottom.png', ...
      '-dpng', '-r300');
...
% Colour limits for relative error
...
% Tick label suppression for column-major
...
% Shared labels
t1.Title.String = 'MLE Estimate Relative Error';
t1.XLabel.String = '\epsilon';
t1.YLabel.String = '\sigma';
t1.Title.FontSize = 24;
t1.XLabel.FontSize = 24;
t1.YLabel.FontSize = 24;
...
% Export
set(fig1, 'PaperPositionMode', 'auto');
print(fig1, 'Figures_withDynamics/heatmap_sweepRate_smallSample_mleRelative_top.png', ...
      '-dpng', '-r300');
...
% Export
set(fig2, 'PaperPositionMode', 'auto');
print(fig2, 'Figures_withDynamics/heatmap_sweepRate_smallSample_mleRelative_bottom.png', ...
      '-dpng', '-r300');
...
% Export
set(fig1, 'PaperPositionMode', 'auto');
print(fig1, 'Figures_withDynamics/heatmap_sweepRate_smallSample_mleAbsolute_top.png', ...
      '-dpng', '-r300');
...
% Export
set(fig2, 'PaperPositionMode', 'auto');
print(fig2, 'Figures_withDynamics/heatmap_sweepRate_smallSample_mleAbsolute_bottom.png', ...
      '-dpng', '-r300');
...
% Colorbar
cb = colorbar;
cb.Ticks = [0 0.01 0.02 0.03 0.04]
cb.Layout.Tile = 'east';
...
% Export
set(fig1, 'PaperPositionMode', 'auto');
print(fig1, 'Figures_withDynamics/heatmap_sweepRate_smallSample_intervalDist_top.png', ...
      '-dpng', '-r300');
...
% Shared labels (same style as previous adaptations)
t2.XLabel.String = '$t$';
t2.XLabel.Interpreter = 'latex';
t2.YLabel.String = '$P(t)$';
t2.YLabel.Interpreter = 'latex';
...
% Export
set(fig2, 'PaperPositionMode', 'auto');
print(fig2, 'Figures_withDynamics/heatmap_sweepRate_smallSample_intervalDist_bottom.png', ...
      '-dpng', '-r300');
