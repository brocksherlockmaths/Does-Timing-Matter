function [data] = logNGenerator_biasedUniform_fixt0(theta,t,sigma,model,epsilon,numSamples)
%UNTITLED Summary of this function goes here
%   epsilon is the upper end of intervalfor uniform biased noise
%   no time error is given to the initial measurement, i.e., time t = t0 is
%   correctly specified
...
if epsilon <= 0 % biased to early measurements
    timeError = unifrnd(epsilon,0,[size(t,1)-1,numSamples]);
else % biased late measurments
    timeError = unifrnd(0,epsilon, [size(t,1)-1,numSamples]);
end
t(2:end,:) = t(2:end,:) + timeError;
